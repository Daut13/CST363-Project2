...
c.sendToAll("reduce select e1.empid, e1.name, e1.salary, d.dept, m1.name, m1.salary "
            "from tempdept d join tempemp e1 "
            "on d.dept = e1.dept "
            "tempemp m1 on d.dept = m1.dept "
            "where e1.salary > (select m1.salary from tempdept d2 join employee m1 "
            "on d2.empid = m1.empid "
            "where d2.dept = e1.dept) "
            "group by e1.empid")
